...
| 业务数据持久化                    | 部分实现         | Project、Device、Device Type 已有 PostgreSQL Repository 与真实数据库集成测试，但尚未注入 HTTP 运行时；其余领域 Repository 仍待实现；`backend/internal/storage/repository/postgres_project_device.go`。        |
