...
verified_against_code_revision: implementation-unit-2026-07-31-command-http
---
...
| 业务数据持久化                    | 部分实现         | Project、凭据版本、Device、Command 创建/取消、对应 Event、初始 Webhook Delivery 与 Audit 已注入 HTTP 运行时；RawMessage、通用 Event/Webhook 查询和 worker、simulator config 尚未统一接入。                             |
...
| Capability 分层                   | 部分实现         | `smart-lock` revision 1 profile 与 Device Type 读取已接入持久运行时；Command 创建从持久 profile 派生 action、payload、revision、delivery policy 与 deadline。dispatcher 尚未消费同一 profile 完成 Provider 映射。    |
...
| Event、Webhook、Audit             | 部分实现         | Auth/Project/Device/Command Audit 已持久化；Device 与 Command 创建/状态变化在同一事务写 Event、配置存在时写初始 Delivery。通用查询、Delivery worker、Attempt 与 manual replay HTTP 仍使用独立内存服务。             |
| Webhook 配置                      | 部分实现         | `/v1/projects` 是唯一配置入口，旧 `/v1/projects/webhook-endpoints` 写入口已移除；URL、AES-256-GCM secret 版本与 Audit 已持久化，Device/Command 初始 Delivery 使用事务内 Project 配置 snapshot；持久 worker 尚未接入。 |
...
| 请求关联与分页                    | 部分实现         | envelope 固定输出全部字段；request ID 已贯穿 header、响应、日志和持久 Auth/Project/Device/Command Audit。Project、Device Type、Provider、Admin/Open Device 与 Command 已实现严格分页和冻结排序，其余列表仍待接入。   |

## 关键实现漂移

### 持久化与模型

- PostgreSQL 当前承载 migration、管理员认证、Project、Device 和 Command aggregate。Project/Device/Command HTTP 已可跨应用实例重建读取；通用 Event/Webhook HTTP 与 worker、simulator 运行态仍由旧内存服务维护并会在重启后丢失。
...
- Project、Device Type、Provider、Device 和 Command list 已提供严格 `page`、`page_size`、`total`、重复/未知 query 拒绝与冻结排序；Event、Webhook Delivery 和 Audit list 尚未对齐该合同。
...
- 持久 Command 创建已与 Device、Provider registry 和发布 profile 统一，且 disabled/deleted Device、Provider 配置、payload 与 Project scope 均在落库前校验；这只证明 API 接受和 `queued` 持久化，不证明 dispatcher 或设备执行。

### Webhook、审计与前端

- Device 与 Command HTTP 已在持久事务内写 Event、配置存在时的初始 Webhook Delivery 和 Audit；通用 Event/Webhook/Audit 管理路由及 worker 仍使用独立内存服务，尚未形成完整统一主链。
- Webhook retry worker 每 500ms 只扫描旧进程内记录，仍未领取 PostgreSQL Delivery/Attempt 或执行持久恢复；Device/Command 已创建的持久初始 Delivery 不会因重启丢失，但当前 worker 也不会投递它们。旧内存路由的记录与重试进度会在重启后丢失，dead resend 仍会重置同一内存记录的计数。Repository 中已验证的独立 replay 历史不能被写成当前 HTTP 行为。
...
- Go 测试覆盖持久 Project HTTP 生命周期、重启读取、一次性凭据披露、凭据轮换、IP whitelist、并发与回滚；也覆盖持久 Device 与 Command 的创建/读取/过滤/lifecycle、Project 隔离、Provider/profile gate、持久幂等与并发、取消、稳定排序，以及 Device/Command、Event、Delivery、Audit 原子回滚。PostgreSQL integration tests 另覆盖 Command 与 Webhook Repository 的 lease、恢复和证据约束；WWTIOT client/callback 只有无真实设备的组件合同证据。
- 前端类型检查、i18n 检查和构建可验证静态一致性。
- 这些检查不能证明持久 Command dispatcher、simulator 主链、Webhook worker、公开可信 callback 或真实智能锁最终执行；Command 创建的 `201`/幂等重放 `200` 只证明平台接受请求并持久化 `queued`，WWTIOT HTTP 测试服务器也不能替代真实设备证据。
