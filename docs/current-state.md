...
verified_against_code_revision: command-evidence-event-implementation-unit
---
...
| Event、Webhook、Audit    | 已实现             | 持久 Event、Delivery/Attempt、Audit、dispatcher 和 dead replay 主链已接入；Provider acceptance 使用 `command.evidence_updated` 表达证据推进，`command.status_changed` 只表达真实状态迁移。                                                                                                                   |
...
| 管理后台                 | 已实现             | Project、Device、Provider/Device Type、Command、Event、Webhook、Audit 和 simulator 页面已接入；DTO、分页 envelope、状态展示、一次性凭据、lifecycle、profile action、cancel、详情、dead replay、Simulator outcome 与 `command.evidence_updated` 筛选/详情均按后端事实工作。                                   |
...
- Provider acceptance 在 Command 保持 `sent` 时写入独立 `command.evidence_updated`，不再制造 `from=sent,to=sent` 的 `command.status_changed`；domain、schema、worker、查询 allowlist、迁移测试和前端筛选均已对齐 `.3` 冻结合同。
...
- Go 测试覆盖持久 Project HTTP 生命周期、重启读取、一次性凭据披露、凭据轮换、IP whitelist、并发与回滚；也覆盖持久 Device 与 Command 的创建/读取/过滤/lifecycle、Project 隔离、Provider/profile gate、持久幂等与并发、取消、稳定排序，以及 Device/Command、Event、Delivery、Audit 原子回滚。PostgreSQL integration tests 另覆盖 Event/Webhook/Audit 严格查询、详情安全 DTO、dead replay 当前配置 snapshot、Webhook 精确签名、逐字节重试、Attempt/dead、secret 解密安全审计、lease 恢复、并发、回滚与重启读取，以及 Command worker 与 simulator 结果矩阵、profile timeout、Provider 公平领取、snapshot 不变性、deadline scanner 和晚结果 fencing；Provider acceptance 另断言 `command.evidence_updated` 的五字段 payload、稳定去重键、完成 Attempt/Command 关联、初始 Delivery raw body 和 Event/Delivery 失败时的事务回滚，并拒绝同状态 `command.status_changed`。安装测试覆盖 marker-only 权威、journal 脱敏/权限/完整性、四阶段恢复、context-aware 文件锁、users 单例/admin-only、两个真实子进程竞争同一 marker/数据库时仅一个成功、两个进程使用不同 marker 目标但共享同一数据库时仍仅一个成功、`admin_pending` 崩溃补偿、marker 前锁所有权丢失补偿，以及 marker-present 或 marker 后清理失败不删除管理员、不回滚配置。其他真实独立 server 子进程测试覆盖跨进程 Command 幂等、Webhook 单一领取，以及进程终止后等待 lease 到期并保守恢复为 `unknown/provider_delivery_unknown`。WWTIOT callback 具有无副作用组件合同证据；WWTIOT 下行另有本地伪 Provider、真实 client registry、已安装应用、持久 worker 与 PostgreSQL 同链集成测试，覆盖 V2 请求/签名、Provider acceptance、Event、观察超时和 transport 诊断脱敏，但仍没有真实服务或设备证据。
