...
verified_against_code_revision: implementation-unit-2026-07-31-local-admin-menu
---
...
- Admin9 模板的用户、角色、菜单、字典、系统日志和用户中心源码、locale 与 `/api/system/**` 客户端均已移除；当前后台只包含冻结目标内的单管理员技术控制台页面。登录页不提供未纳入合同的自助密码找回或重置入口，相关 URL 按未知前端路由处理。菜单固定由前端冻结路由生成，不提供服务端菜单开关，旧 `/v1/auth/menu` 空占位端点已移除。
