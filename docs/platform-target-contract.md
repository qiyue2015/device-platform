...
建设一个可实际运行、可持久化、状态可信的 IoT 设备平台，使共享单车应用能够通过一个 Project 机器身份，使用同一条平台主链接入 WWTIOT 厂商云智能锁与 Omni 直连智能锁。
...
Project、Device、Command、Attempt、Result、DeviceState、RawMessage、Event、Webhook Delivery 和 Audit 的权威事实必须持久化。进程或消息基础设施重启后，未完成工作可恢复；重复请求、重复消息、乱序或迟到结果不能产生重复物理效果或覆盖不可变历史。

### 模拟器与管理后台

模拟器必须通过同一持久命令主链，只替换 Provider adapter 的受控输出。它用于验证 Core，不提供真实 Device ACK/final evidence，也不替代真实设备验收。

管理后台是单管理员技术控制台，读取与 Open API、worker 相同的权威事实。它用于 Project、Device、Provider、Command、Result、Event、Webhook 和 Audit 的管理诊断，不承担共享单车运营业务。

## 不进入本次完成定义

- Omni 资料所含但未进入 smart-lock 当前三项 action 的 BLE 控制、地图、通用位置、电子围栏、OTA、固件管理、规则引擎、批量操作、告警和长期遥测保留；
- 自行车、景区、用户、订单、行程、计费、停车、还车和业务权限；
- 多租户 SaaS、组织员工体系、复杂人类 RBAC、低代码平台或通用业务工作流。
...
1. Project 机器认证与隔离、Command 幂等、领域状态机、事务边界、Outbox/Inbox、Webhook 和 Audit 按核心合同实现并自动化验证。
...
5. API、后台、worker 和持久化层读取同一权威模型；核心合同有自动化一致性与故障测试。
