...
func TestRuntimeAndInstallRejectDriftedFrozenProfile(t *testing.T) {
	...
	_, err = performInstall(context.Background(), setupInstallRequest{
		...
		Redis:    redisSetupRequest{URL: requireProcessRedisTestURL(t)},
...
func TestNewAppRejectsDatabaseMissingLatestMigration(t *testing.T) {
	withAuthTestDatabase(t, func(db *sql.DB) {
		...
		if err == nil || !strings.Contains(err.Error(), "database migration validation failed") ||
			!strings.Contains(err.Error(), "missing=[010_multi_user_project_authorization]") {
			t.Fatalf("newApp must reject a database missing migration 010, got %v", err)
		}
	})
}
