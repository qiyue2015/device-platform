...
func validAuditAction(value string) bool {
	switch value {
	case "setup.completed", "auth.login", "auth.refresh", "auth.logout",
		...
		"command.created", "command.cancelled", "provider.callback_rejected", "webhook.delivery_replayed", "simulator.updated":
		return true
	default:
		return false
	}
