...
- `make migrate-up`, `make migrate-down`, and setup all use the repository runner
  and its `schema_migrations(version, applied_at)` table. Do not run the
  external `golang-migrate` CLI against this database.
